...
//        File buildingFile = new File(rootFolder, "IGN/batVeme.gpkg");
        DataStore ds = CollecMgmt.getDataStore(new File(rootFolder,"IGN/batVeme.gpkg"));
...
//        createMultiBlock(blockFile, buildingFile, MergeGoal.LGT, 200, new File("/tmp/"));
    ...
    public static File createMultiBlock(File blockFile, File goalFile, MergeGoal mergeGoal, double threshold, File resultFolder) throws IOException {
        DataStore dsBlock = CollecMgmt.getDataStore(blockFile);
        ...
        switch (mergeGoal) {
            case LGT:
                if (!CollecMgmt.isCollecContainsAttribute(blockSFC, "NB_LOGTS-SUM")) {
                    DataStore buildingDS = CollecMgmt.getDataStore(goalFile);
                ...
                break;
            case POP:
                if (!CollecMgmt.isCollecContainsAttribute(blockSFC, "countPOP")) {
                    DataStore popDS = CollecMgmt.getDataStore(goalFile);
        ...
        CollecMgmt.exportSFC(blockSFC,new File("/tmp/exBock.gpkg"));
