...
//    }
     ...
     *
     * @param amenityCode source amentiy code
     * @param from        the name of the class TODO change in class would be more clean
     * @return the <i>Ici Amenity</i> nomenclature
     */
    public static String getIciAmenity(String amenityCode, String from) throws InvalidPropertiesFormatException {
        try {
                ...
                if (lP.size() > 1) {
                    ...
                    for (POI pObs : lP) {
                        if (alreadyAdd.contains(pObs))
                            continue;
                        // If the POI is of the same class as every other ones, we add it
